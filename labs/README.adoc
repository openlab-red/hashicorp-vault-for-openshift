...
This lab walks you through the process of build  HashiCorp Vault in OpenShift.
...
Welcome to the RHEL8 Gold Image for Azure Lab! FIXME
...
Web console: https://workstation-GUID.rhpds.opentlc.com:9090/ or
https://10.0.0.3:9090/
...
[NOTE]
The default prompt has been configured to use colors, just because it can be easier finding what you've typed when following lab instructions. This is not a normal RHEL8 feature and is just used in this lab. If you would like to remove the prompt colors, delete the `/etc/profile.d/profile.sh` file and login again.
